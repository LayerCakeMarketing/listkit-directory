...
const props = defineProps({
    ...
    maxFileSize: {
        type: Number,
        default: 14680064 // 14MB in bytes (Cloudflare max)
    },
    acceptedTypes: {
        type: String,
        default: 'image/*'
    ...
    context: {
        type: String,
        default: null,
        validator: (value) => !value || ['avatar', 'cover', 'gallery', 'logo', 'item'].includes(value)
    },
    entityType: {
        type: String,
        default: null
...
const uploadFiles = async () => {
    ...
    try {
        ...
        console.log('Upload URL response headers:', response.headers)
        
        if (!response.ok) {
            const errorText = await response.text()
            console.error('Upload URL error response:', errorText)
            throw new Error(`Failed to get upload URL: ${response.status} - ${errorText.substring(0, 200)}`)
        }
        ...
        try {
            ...
            console.error('Failed to parse upload URL response as JSON:', parseError)
            console.error('Response text:', responseText)
            throw new Error('Server returned invalid JSON response for upload URL')
        }
                ...
                if (xhr.status === 200) {
                    try {
                        ...
                        fileObj.result = {
                            ...
                            url: response.result.variants?.[0] || `https://imagedelivery.net/nCX0WluV4kb4MYRWgWWi4A/${response.result.id}/public`,
                            filename: fileObj.name
                        }
...
const confirmUpload = async (uploadResult) => {
    try {
        ...
        const response = await fetch('/api/cloudflare/confirm-upload', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        ...
        if (!response.ok) {
            console.warn('Failed to confirm upload in database:', response.status)
        } else {
            const data = await response.json()
            console.log('Upload confirmed in database:', data)
        }
    } catch (error) {
